...
export async function skapaOrder(_prev: Orderstate, form: FormData): Promise<Orderstate> {
  try {
    ...
    return {
      ok: efterslapning
        ? `Ordern på ${bolag} är godkänd. ${manadsnamn(periodFor(signerad))} var fastställd, så provisionen bokfördes på ${manadsnamn(efterslapning)}.`
        : godkannDirekt
...
export async function godkannOrder(_prev: Orderstate, form: FormData): Promise<Orderstate> {
  try {
    ...
    return {
      ok: efterslapning
        ? `Ordern är godkänd. ${manadsnamn(periodFor(rad.signed_on))} var fastställd, så ${kronor(provision.satt.commission_amount)} bokfördes på ${manadsnamn(efterslapning)} med en anteckning om varför.`
        : "Ordern är godkänd och räknas från och med nu.",
    };
  } catch (e) {
    return { fel: e instanceof Error ? e.message : "Något gick fel." };
  }
