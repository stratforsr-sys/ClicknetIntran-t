...
- **MERGEN ÅTERSTÅR, och den är hela påslaget.** Navnyheten i commiten säger att
  inspelningarna har flyttat; det blir sant i samma stund grenen går in i main.
- **Ingen inspelning ligger ännu i R2.** Provfilen är städad. Första riktiga
  samtalet efter mergen är värt att spela upp för hand — läsvägen för en
  R2-fil har aldrig gått genom `signeraOchLogga` mot riktig data, bara genom
  samma SDK-anrop i `prova-r2.mjs`.
- **Kontrollera efter mergen att nya rader bär `store = 'r2'`.** En rad med
  `store = 'supabase'` efter påslaget betyder att R2 föll och Supabase tog emot
  — ljudet finns, men något är fel med nycklarna.
