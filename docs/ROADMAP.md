...
| E0.7 | **Strukturerad loggning och larm vid integrationsfel** | KLAR 2026-08-27 — `onRequestError` skrev serverfelen redan 2026-08-22; nu larmar **nattjobbet om sig självt**. Varje fallet steg blir en rad i `error_report` med en digest som är stabil över nätter (`src/lib/jobb/larm.ts`), så en månads haveri blir en rad med räknaren 30. Jobbet kontrollerar dessutom sitt eget föregående kvitto — äldre än 26 timmar betyder att en natt hoppats över. **Färskhetskontrollen ligger på en mänsklig väg, inte i en andra cron:** driftkort på `/fel` och en rad på startsidan som bara ritas när något är fel. En cron som vaktar cron dör samma död — se D-E0.7. Kvarstår när A5/A6 besvaras: integrationerna själva, som ännu inte finns |
...
| E1.5 | AC-1.3 | **Automatisk tilldelning** av rutiner, kurser, schema och team vid upplägg | **KLAR 2026-08-27.** Alla fyra. `/personal/ny` och anställningsflödet (E10.9) anropar **samma** `laggUppAnstalld()` i lib, så de kan inte glida isär. Rutiner, kurser och team var klara; **schemat behövde ingen kod** — `work_schedule` har scope `company` och `gallandeSchema()` väljer den mest specifika, så bolagsschemat gäller varje nyanställd från dag ett. En rad per person hade varit skadlig: ett schema ändras genom en ny rad med nytt `valid_from` (AC-2.35), och tjugofem kopior hade blivit tjugofem ändringar. **Det som saknades var beviset** — `onboarding.schedule_assigned` skrivs nu som rutiner och kurser redan gjorde, **och även när inget schema gäller**, vilket chefen varnas om på kvittot |
...
| # | Vad | Status |
|---|---|---|
| X1 | **WCAG 2.1 AA**: kontrast, tangentbord, fokus, `aria-describedby` på formulärfel | **Genomgången 2026-08-27.** Kontrast mätt sedan D-U2/D-U3, `:focus-visible` global med brand-600 och offset, `Field`/`Input`/`Select` kopplar fel med `aria-describedby` + `aria-invalid`, ikoner är `aria-hidden`, `<html lang="sv">`. **Två verkliga brister rättade:** ingen hoppa-till-innehåll-länk (2.4.1, nivå A — sjutton menylänkar före innehållet på varje sida), och felnotiser annonserades `polite` i stället för `assertive` (4.1.3). Kvar innan punkten stängs helt: en genomgång med riktig skärmläsare, som inte går att göra från kommandoraden |
| X2 | **375 px** fungerar för stämpling, ledighet, rutiner, kurser, ärenden | **Genomgången 2026-08-27.** Samtliga breda tabeller ligger redan i `overflow-x-auto`-behållare, så sidan i sig skrollar aldrig i sidled: `/kv`, `/tid/avvikelser`, `/tid/ogiltig-franvaro`, `/admin/arbetstid` och lönerapporten. Bottennavigering under 768 px, sidopanelens meny scrollar (2026-08-24). Inga nya brister hittade i granskningen. Kvar: en avläsning på riktig telefon, som inte går att göra från kommandoraden |
